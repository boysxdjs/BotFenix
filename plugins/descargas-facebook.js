...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  if (!args[0]) throw `*[ ℹ️ ] Ingrese un enlace de Facebook.*\n\n*[ ℹ️ ] Ejemplo:* _${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/ _`;
  if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*[ ℹ️ ] Ingrese un enlace de Facebook.*\n\n*[ ℹ️ ] Ejemplo:* _${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/ _`;
  try {
    await m.reply(`*[ ℹ️ ] Se está enviando el video. espere...`);
    const d2ata = await facebook.v1(args[0]);
    let r2es = '';
    if (d2ata.urls && d2ata.urls.length > 0) {
      r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`;
    }
    conn.sendFile(m.chat, r2es, 'error.mp4', `*[ 📥 ] Descargas - Facebook*\n_---> 🦂 𝘼𝙡𝙖𝙘𝙧𝙖𝙣 🦂._`, m);
  } catch (err1) {
    try {
      ...
      try {
        ...
        conn.sendFile(m.chat, VIDEO, 'error.mp4', `*[ 📥 ] Descargas - Facebook*\n_---> 🦂 𝘼𝙡𝙖𝙘𝙧𝙖𝙣 🦂._`, m);
      } catch (err2) {
        try {
          const ress = await fg.fbdl(args[0]);
          const urll = await ress.data[0].url;
          await conn.sendFile(m.chat, urll, 'error.mp4', '*[ 📥 ] Descargas - Facebook*\n_---> 🦂 𝘼𝙡𝙖𝙘𝙧𝙖𝙣 🦂._', m);
        } catch (err3) {
          try {
            const res = await fbDownloader(args[0]);
            for (const result of res.download) {
              const ur = result.url;
              await conn.sendFile(m.chat, ur, 'error.mp4', '*[ 📥 ] Descargas - Facebook*\n_---> 🦂 𝘼𝙡𝙖𝙘𝙧𝙖𝙣 🦂._', m);
            }
          } catch (err4) {
            try {
              ...
              await conn.sendFile(m.chat, url3, 'error.mp4', '*[ 📥 ] Descargas - Facebook*\n_---> 🦂 𝘼𝙡𝙖𝙘𝙧𝙖𝙣 🦂._', m);
            } catch (err5) {
              try {
